...
if wait_for_descendant "$copilot_pane_shell_pid" 30 >/dev/null; then
	...
	if [ -n "$copilot_sid" ]; then
		...
		# A resumable session needs real content, which needs an authenticated
		# API call. The UUID and the PID mapping above are entirely real; this
		# one file stands in for "the user has typed something", so the save
		# hook's resumability gate is satisfied. The content marker moved across
		# versions -- session.db through ~1.0.78, a non-empty events.jsonl on
		# 1.0.88+ -- and the gate accepts either; we stand in the current one.
		# test/copilot-contract-test.sh pins the real behaviour on both sides.
		printf '{"type":"user"}\n' >"$(dirname "$copilot_lock")/events.jsonl"
	else
